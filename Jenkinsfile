...
//             }
            steps {
            	script {
					...
					sh "cd cms-transforms-c-sharp/CmsTransformLibrary/bin/Release/ && sudo dotnet nuget push ${src} -s https://monetate.jfrog.io/artifactory/api/nuget/v3/dotnet-local -k ${key}"
                }
            }
        }
    }
